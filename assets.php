...
function assets() {
  ...
  // add eventlistener polyfill for ie8 and below
  if(preg_match('/(?i)msie [4-7]/',$_SERVER['HTTP_USER_AGENT']))
  {
      // if IE<=7
      wp_enqueue_script('eventlistener', asset_path('scripts/eventlistener.oldie.js'), [], null, false);
  }
  elseif(preg_match('/(?i)msie [8]/',$_SERVER['HTTP_USER_AGENT']))
  {
      // if IE=8
      wp_enqueue_script('eventlistener', asset_path('scripts/eventlistener.js'), [], null, false);
  }

  // add respond.js for ie8 and below
